...
def _infer_currency_unit(*texts: Optional[str]) -> Optional[str]:
    """Infer a common currency unit from free-form text snippets.

    Parameters
    ----------
    *texts :
        Candidate strings such as column names, descriptions, and dataset labels.

    Returns
    -------
    Optional[str]
        The detected ISO-like currency code (currently only ``"USD"``) or ``None``.
    """
...
def _load_preview(
    ...
    """Read a small preview of the dataset and basic numeric stats.

    Parameters
    ----------
    metadata :
        Catalog metadata for the dataset being analysed.
    max_rows :
        Maximum number of preview rows to retain.

    Returns
    -------
    (list[dict[str, str]], int, dict[str, NumericSummary])
        Preview rows, observed row count, and numeric column summaries.
    """
...
class AnalysisEngine:
    ...
    def analyze(self, question: str, datasets: Sequence[DatasetMetadata]) -> QuestionAnalysis:
        """Compile dataset summaries and optional PandasAI insights for a question.

        Parameters
        ----------
        question :
            Raw user question extracted from Slack.
        datasets :
            Candidate datasets selected by the router.

        Returns
        -------
        QuestionAnalysis
            Aggregated context including previews, highlights, insights, and charts.
        """
    ...
    def _load_dataframe(
        ...
        """Load a Pandas DataFrame for PandasAI while enforcing optional row limits."""
    ...
    def _capture_charts(self, captured: List[str]):
        """Context manager that intercepts Matplotlib chart outputs.

        Captures charts generated by PandasAI by temporarily patching
        ``plt.show`` and ``plt.savefig``. Any successfully persisted figures are
        appended to ``captured`` so they can be uploaded to Slack later.
        """
        try:  # pragma: no cover - optional dependency
            import matplotlib
        ...
        original_savefig = getattr(plt, "savefig", None)
        skip_next_savefig_capture = False

        def _record_chart_path(candidate: object) -> None:
            ...
            if not resolved.exists():
                return
            captured.append(str(resolved))

        def _patched_savefig(*args, **kwargs):  # type: ignore[no-untyped-def]
            nonlocal skip_next_savefig_capture
            ...
            if skip_next_savefig_capture:
                return result
            # Record files created through explicit savefig calls during execution.
            _record_chart_path(fname)
            return result
        ...
        def _patched_show(*args, **kwargs):  # type: ignore[no-untyped-def]
            ...
            try:
                skip_next_savefig_capture = True
                try:
                    # Persist implicit plt.show() calls to a deterministic location.
    ...
    def _normalize_insight_output(
        ...
        """Clean PandasAI output and deduplicate chart references.

        Parameters
        ----------
        answer :
            Raw textual response from PandasAI.
        captured_charts :
            Chart paths detected during execution.

        Returns
        -------
        tuple
            The sanitised answer (or ``None``) and a tuple of unique chart paths.
        """
    ...
    def _merge_chart_paths(
        ...
        """Merge chart paths while preserving the first occurrence of each file."""
    ...
    def _generate_insight(
        ...
        Returns both the natural-language answer (if available) and execution
        metadata so callers can display SQL traces and charts.
        """
        try:
